...
const format=async()=>{const [me,status]=await Promise.all([api('/api/me'),api('/api/status')]);const label=path==='overview'?'Overview':nav.find(x=>x[0]===path)?.[1]||'Developer';$('user').innerHTML='Signed in as <b>'+me.username+'</b><a class="logout" href="/auth/logout">Sign out</a>';$('sidebar-greeting').textContent='Hi, '+me.username;$('route-label').textContent='Dashboard / '+label;$('title').textContent=label;render(path,status)};
