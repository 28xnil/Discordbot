...
function violation(content, settings) {
    const lower = content.toLowerCase();
    if (settings.blockInvites && /(discord\.gg|discord(?:app)?\.com\/invite)\//i.test(content))
        return 'Discord invite links are not allowed.';
    if (settings.blockLinks && /https?:\/\//i.test(content))
        return 'Links are not allowed here.';
    if (settings.blockCaps && hasExcessiveCaps(content))
        return 'Please avoid excessive capital letters.';
    if (settings.blockMentions && (content.match(/<@!?(\d+)>/g)?.length ?? 0) >= 6)
        return 'Mention spam is not allowed.';
    if (settings.blockedWords.some((word) => word && lower.includes(word)))
