...
function getTarget(interaction: Parameters<Command['execute']>[0], name = 'user'): GuildMember | null {
  return interaction.options.getMember(name) as GuildMember | null;
}

function getReason(interaction: Parameters<Command['execute']>[0]): string {
...
const warn: Command = {
  ...
  async execute(interaction) {
    ...
    else if (total >= 3) await target.timeout(10 * 60 * 1000, `Automatic escalation: ${total} warning points`);
  }
};
...
async function moderateMember(interaction: Parameters<Command['execute']>[0], action: 'TIMEOUT' | 'KICK' | 'BAN') {
  if (!(await requireStaff(interaction)) || !interaction.guild) return;
  const target = getTarget(interaction);
  if (!target) return interaction.reply({ embeds: [errorEmbed('That member is not in this server.')], ephemeral: true });
  if (target.id === interaction.user.id || target.id === interaction.guild.ownerId) return interaction.reply({ embeds: [errorEmbed('You cannot moderate that member.')], ephemeral: true });
  const reason = getReason(interaction);
...
export const moderationCommands: Command[] = [warn, warnings, clearWarnings, unwarn, timeout, untimeout, kick, ban, unban, cases, caseCommand];
