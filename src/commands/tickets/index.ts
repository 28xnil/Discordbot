...
import { createTicket, getGuildTickets, getOpenTicketForUser, getTicketByChannel, getTicketConfig, updateTicket, updateTicketConfig } from '../../database.js';
...
type TopicEmoji = { id?: string; name?: string; animated?: boolean; unicode?: string };

function parseTopic(value: string): { name: string; emoji?: TopicEmoji } {
  ...
  if (nameParts.length && emoji.trim()) return { emoji: { unicode: emoji.trim() }, name: nameParts.join('|').trim() };
  return { name: value.trim() };
}

function topicValue(value: string): string {
...
function ticketPanel(topics: string[]) {
  ...
  return [
    ...
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId(ticketPanelButtonId).setLabel('General support ticket').setStyle(ButtonStyle.Secondary)
    )
  ];
}
...
async function createTicketChannel(interaction: ChatInputCommandInteraction | ButtonInteraction | StringSelectMenuInteraction, topic?: string) {
  ...
  if (existing) {
    ...
    if (interaction.isButton()) await interaction.reply({ embeds: [message], ephemeral: true });
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    ...
    new ButtonBuilder().setCustomId('ticket:claim').setLabel('Claim ticket').setStyle(ButtonStyle.Success)
  );
  ...
  if (ticketConfig.logChannelId) {
    ...
    if (logChannel?.isTextBased()) await logChannel.send({ embeds: [createEmbed('Ticket created', `${channel} opened by ${interaction.user}.`, colors.success)] });
  }
  const confirmation = createEmbed('Ticket created', `Your private ticket is ready: ${channel}`, colors.success);
  if (interaction.isButton()) await interaction.reply({ embeds: [confirmation], ephemeral: true });
  else await interaction.reply({ embeds: [confirmation], ephemeral: true });
}

export async function handleTicketButton(interaction: ButtonInteraction | StringSelectMenuInteraction): Promise<void> {
  if (!interaction.guild) return;
  ...
  if (interaction.customId === 'ticket:claim') {
    if (!interaction.member || !('permissions' in interaction.member) || !isStaff(interaction.member as GuildMember)) {
  ...
  if (interaction.customId === 'ticket:close') {
    if (ticket.userId !== interaction.user.id && !(interaction.member as GuildMember).permissions.has(PermissionFlagsBits.ManageChannels)) {
  ...
  data: new SlashCommandBuilder().setName('ticket').setDescription('Manage support tickets.')
    ...
    .addSubcommand((command) => command.setName('config').setDescription('Configure ticket behavior.')
      .addStringOption((option) => option.setName('setting').setDescription('Setting to change.').setRequired(true).addChoices(
        { name: 'Category', value: 'category' }, { name: 'Staff role', value: 'staff-role' }, { name: 'Log channel', value: 'logs' }, { name: 'Naming pattern', value: 'naming' }, { name: 'User limit', value: 'limit' }, { name: 'Topics', value: 'topics' }
      ))
...
export const ticketCommands: Command[] = [ticketCommand];
