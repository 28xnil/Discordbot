...
function userOption(command: SlashCommandBuilder, description = 'User to inspect.') {
...
export const bannerCommand: Command = {
  ...
  async execute(interaction) {
    ...
    if (!banner) return interaction.reply({ embeds: [errorEmbed(`${user.tag} does not have a profile banner.`)], ephemeral: true });
...
export const chooseCommand: Command = {
  ...
  async execute(interaction) {
    ...
    if (options.length < 2) return interaction.reply({ embeds: [errorEmbed('Provide at least two comma-separated options.')], ephemeral: true });
...
export const remindCommand: Command = {
  ...
  async execute(interaction) {
    ...
    if (!duration) return interaction.reply({ embeds: [errorEmbed('Use a duration like `10m`, `2h`, or `1d`. The maximum is 7 days.')], ephemeral: true });
...
export const utilityCommands: Command[] = [botInfoCommand, serverInfoCommand, userInfoCommand, avatarCommand, bannerCommand, roleInfoCommand, channelInfoCommand, permissionsCommand, memberCountCommand, chooseCommand, rollCommand, pollCommand, remindCommand];
