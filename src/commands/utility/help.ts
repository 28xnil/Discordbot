...
export function helpSectionEmbed(id: string) {
  const section = sections[id as keyof typeof sections] ?? sections['help:moderation'];
...
function helpButtons() {
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId('help:moderation').setLabel('Moderation').setStyle(ButtonStyle.Danger),
    new ButtonBuilder().setCustomId('help:tickets').setLabel('Tickets').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('help:management').setLabel('Management').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('help:utility').setLabel('Utility').setStyle(ButtonStyle.Success)
  );
}
...
export const helpCommand: Command = {
  ...
  async execute(interaction) {
    const embed = createEmbed('Sentinel command centre', 'Choose a section below to see its commands and capabilities.')
      .addFields(
        { name: 'Moderation', value: 'Warnings, punishments, cases, and staff controls.', inline: true },
        { name: 'Tickets', value: 'Private support channels and assignment.', inline: true },
        { name: 'Management', value: 'Server settings and automation.', inline: true },
        { name: 'Utility', value: 'Information and everyday tools.', inline: true }
      );
