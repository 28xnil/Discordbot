...
export const roleCommands: Command[] = [
  {
    ...
    async execute(interaction) {
      ...
      if (subcommand === 'create') {
        ...
        if (role.managed || role.position >= (interaction.guild.members.me?.roles.highest.position ?? 0)) return interaction.reply({ embeds: [errorEmbed('I cannot manage that role.')], ephemeral: true });
