  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      if (url.pathname === '/auth/discord') {
        if (!config.oauthClientSecret) return sendJson(response, { error: 'DISCORD_OAUTH_CLIENT_SECRET is not configured.' }, 503);
        const state = randomBytes(24).toString('hex');
        pendingStates.add(state);
        const oauth = new URL('https://discord.com/oauth2/authorize');
        oauth.searchParams.set('client_id', config.clientId); oauth.searchParams.set('response_type', 'code'); oauth.searchParams.set('redirect_uri', `${config.dashboardUrl}/auth/callback`); oauth.searchParams.set('scope', 'identify'); oauth.searchParams.set('state', state);
        redirect(response, oauth.toString());
        return;
      }
      if (url.pathname === '/auth/callback') {
        if (!config.oauthClientSecret || !url.searchParams.get('code') || !pendingStates.delete(url.searchParams.get('state') ?? '')) return sendJson(response, { error: 'Invalid OAuth callback.' }, 400);
        ...
        if (!token.access_token) return sendJson(response, { error: 'Discord authorization failed.' }, 401);
      ...
      if (url.pathname.startsWith('/api/')) {
        ...
        if (url.pathname === '/api/custom-commands' && request.method === 'GET') return sendJson(response, await getCustomCommands());
        if (url.pathname === '/api/custom-commands' && request.method === 'POST') {
          const input = await body(request);
          if (!/^[a-z0-9-]{1,32}$/.test(input.name ?? '') || !(input.description ?? '').trim() || !(input.response ?? '').trim()) return sendJson(response, { error: 'Name, description, and response are required.' }, 400);
    ...
    } catch (error) { console.error('Dashboard request failed', error); sendJson(response, { error: 'Dashboard request failed' }, 500); }
  });
  server.listen(config.port, '0.0.0.0', () => console.log(`Sentinel dashboard listening on port ${config.port}`));
}

function formatUptime(milliseconds: number): string { const totalMinutes = Math.floor(milliseconds / 60_000); const days = Math.floor(totalMinutes / 1_440); const hours = Math.floor((totalMinutes % 1_440) / 60); const minutes = totalMinutes % 60; return `${days}d ${hours}h ${minutes}m`; }
