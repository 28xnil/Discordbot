...
function caseFromRow(row: Record<string, unknown>): ModerationCase {
  return { id: Number(row.id), guildId: String(row.guild_id), userId: String(row.user_id), moderatorId: String(row.moderator_id), type: row.type as ModerationCaseType, reason: String(row.reason), points: Number(row.points), createdAt: new Date(String(row.created_at)).toISOString() };
}

function ticketFromRow(row: Record<string, unknown>): Ticket {
  return { id: Number(row.id), guildId: String(row.guild_id), channelId: String(row.channel_id), userId: String(row.user_id), topic: row.topic ? String(row.topic) : undefined, claimedBy: row.claimed_by ? String(row.claimed_by) : undefined, status: row.status as Ticket['status'], createdAt: new Date(String(row.created_at)).toISOString(), closedAt: row.closed_at ? new Date(String(row.closed_at)).toISOString() : undefined };
}

export async function addCase(input: Omit<ModerationCase, 'id' | 'createdAt'>): Promise<ModerationCase> {
...
export async function getCase(id: number): Promise<ModerationCase | undefined> {
...
export async function getUserCases(guildId: string, userId: string): Promise<ModerationCase[]> {
...
export async function removeCase(id: number): Promise<boolean> {
  const result = await pool.query('DELETE FROM moderation_cases WHERE id = $1', [id]);
  return result.rowCount === 1;
}

export async function totalWarningPoints(guildId: string, userId: string): Promise<number> {
...
async function getGuildSettings(guildId: string): Promise<GuildSettings> {
  ...
  return (result.rows[0]?.settings ?? {}) as GuildSettings;
}

async function updateGuildSettings(guildId: string, changes: GuildSettings): Promise<void> {
...
export async function updateTicketConfig(guildId: string, changes: Partial<TicketConfig>): Promise<TicketConfig> {
...
export async function updateAutoModConfig(guildId: string, changes: Partial<AutoModConfig>): Promise<AutoModConfig> {
...
export async function createTicket(input: Omit<Ticket, 'id' | 'createdAt' | 'status'>): Promise<Ticket> {
...
export async function getGuildTickets(guildId: string, status?: Ticket['status']): Promise<Ticket[]> {
...
export async function getDashboardData(): Promise<{ commands: number; openTickets: number; totalTickets: number }> {
